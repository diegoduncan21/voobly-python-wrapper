...
class Voobly(object):
    ...
    def init(self):
        ...
        self.get_user = partial(VooblyActionGetUserInformation().act,
                                self._voobly_api_path,
                                self._key)
        self.get_ladder = partial(VooblyActionGetLadderInformation().act,
                                  self._voobly_api_path,
                                  self._key)
...
class VooblyAction(object):
    ...
    def act(self, base_url, key, input, parameters):
        if any([p not in self._allowed_parameters for p in parameters]):
            raise InvalidActionParameter("Allowed parameters are %s, got %s" % (
                self._allowed_parameters,
                parameters))

        requested_url = "%s/%s%s?key=%s%s" % (
            base_url,
            self._action_path,
            input,
            key, "".join(["&%s=%s" % p for p in parameters.iteritems()]))
...
class VooblyActionGetLadderInformation(VooblyAction):
    def __init__(self):
        super(VooblyActionGetLadderInformation, self).__init__(
            "ladder",
            ["uid", "uidlist", "start", "limit"])
    ...
    def act(self, base_url, key, ladder_id, uid=None, uidlist=None, start=None, limit=None):
        if uidlist is not None and uid is not None:
            raise InvalidActionParameter(
                'only one of uid or uidlist parameters should be suplied to ladder, not both.')
        parameters = {}
        ...
        if limit is not None:
            ...
            if limit > MAX_LADDER_LIMIT:
                raise InvalidActionParameter(
                    'max limit to ladder is %d, but received %d.' % (MAX_LADDER_LIMIT, limit,))

        return super(VooblyActionGetLadderInformation, self).act(base_url,
                                                                 key,
                                                                 '/%d' % ladder_id,
                                                                 parameters)
...
class VooblyActionFindUsers(VooblyAction):
    ...
    def act(self, base_url, key, user_names):
        return super(VooblyActionFindUsers, self).act(base_url,
                                                      key,
                                                      '/%s' % ','.join(user_names),
                                                      {})
